...
.get("", async (req, res) => {
	...
	if (!req.user.admin) { user = req.user.id + ""; }

	if (user) {
		return res.send((await DB.query(`
...
.post("", isAdmin, body("name").exists(), validate, async (req, res) => {
	try {
		const result = await DB.query("insert into newsletter (name) values ($1) returning name, newsletter_id", [req.body.name]);
...
.delete("/:newsletterId", query("user").if(query("user").exists).isNumeric(), validate, async (req, res) => {
	...
	try {
		const result = await DB.query("delete from subscription where subscriber_id = $1 and newsletter_id = $2", [user, req.params.newsletterId]);
