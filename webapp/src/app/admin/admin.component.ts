...
export class AdminComponent {
	...
	constructor(private adminService: AdminService, private authService: AuthService) {
		...
		if (!this.admin) {
			...
			adminService.getUsers().subscribe(users => {
				this.users = users as any[];
			}, err => {

			});
			adminService.getLetters().subscribe(letters => {
	...
	addLetter() {
		...
		if (name) {
			this.adminService.newLetter(name).subscribe(letter => {
	...
	addUser() {
		...
		if (name) {
			this.adminService.newUser(name, pass).subscribe(newUser => {
	...
	selectUser(id: number) {
		...
		if (this.selectedUser === id) {
			...
			this.letters = [];
			this.adminService.getLetters(this.selectedUser).subscribe(letters => {
				this.letters = letters as any[];
			}, err => {
